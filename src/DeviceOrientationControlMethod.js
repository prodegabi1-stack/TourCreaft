...
window.Marzipano.dependencies.eventEmitter(DeviceOrientationControlMethod);
...
DeviceOrientationControlMethod.prototype._handleData = function(data) {
  ...
  // Apply Low-Pass Filter (EMA) to smooth out sensor jitter
  var alpha = 0.20; // 0.20 gives a buttery smooth delay without feeling laggy
  if (previous.yaw != null) {
      var dy = current.yaw - previous.yaw;
      // Normalize dy to [-PI, PI] to prevent wrap-around bugs during smoothing
      while (dy > Math.PI) dy -= 2 * Math.PI;
      while (dy < -Math.PI) dy += 2 * Math.PI;
      
      current.yaw = previous.yaw + alpha * dy;
      current.pitch = previous.pitch + alpha * (current.pitch - previous.pitch);
      current.roll = previous.roll + alpha * (current.roll - previous.roll);
      
      // Normalize yaw back just in case
      while (current.yaw > Math.PI) current.yaw -= 2 * Math.PI;
      while (current.yaw < -Math.PI) current.yaw += 2 * Math.PI;
  }
